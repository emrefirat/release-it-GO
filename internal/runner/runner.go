...
func (r *Runner) init() error {
	...
	// Repository info comes from the remote we release to (git.pushRepo,
	// default origin), like npm release-it.
	repoInfo, err := git.GetRepoInfo(r.ctx.Config.Git.PushRepo)
	if err != nil {
		r.ctx.Logger.Verbose("Could not get repo info: %v", err)
		// Non-fatal here: local-only releases don't need it. Prerequisites
		// turn it into an error when a platform release is enabled.
	} else {
...
func (r *Runner) checkPrerequisites() error {
	...
	// After the token check: a missing token is the more actionable error.
	if err := r.checkRepoInfoForReleases(); err != nil {
		r.ctx.Spinner.Stop(false)
		return err
	}
...
// checkRepoInfoForReleases fails early when a GitHub/GitLab release is enabled
// but the remote could not be parsed: the release steps used to skip
// silently and the run still printed "Done".
func (r *Runner) checkRepoInfoForReleases() error {
	cfg := r.ctx.Config
	if !cfg.GitHub.Release && !cfg.GitLab.Release {
		return nil
	}
	if r.ctx.RepoInfo != nil {
		return nil
	}
	remote := cfg.Git.PushRepo
	if remote == "" {
		remote = "origin"
	}
	platform := "GitHub"
	...
	return fmt.Errorf("%s release is enabled but the %q remote is missing or its URL is not recognized (set git.pushRepo or fix the remote URL)", platform, remote)
}
...
func (r *Runner) autoDetectIncrement() string {
	rawCommits, err := r.commitsSinceLatestRelease()
	if err != nil {
		// Falling back to "patch" silently produced wrong-but-successful runs.
		r.ctx.Logger.Warn("could not auto-detect the increment (falling back to patch): %v", err)
		return "patch"
	}
	if len(rawCommits) == 0 {
		return "patch"
...
// stageReleaseFiles stages tracked modifications (git add . --update), like
// npm release-it: with requireCleanWorkingDir the tree was clean when the
// release started, so everything modified since — bumper outputs, the
// changelog, and files regenerated by hooks (dist/, lockfiles) — belongs to
// the release commit. Staging only the bumper outputs (Phase 26) dropped
// hook-produced changes and left the tree dirty. addUntrackedFiles also
// sweeps in new files (git add .).
